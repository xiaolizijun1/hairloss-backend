# backend/train_classification.py
...
def map_yes_no(series):
    ...
    }).fillna(series).apply(lambda v: 1 if str(v).strip() in ("1","1.0") else (0 if str(v).strip() in ("0","0.0","") else v))
...
if "Hair_Loss" not in df.columns:
    if "HairLoss" in df.columns:
        ...
        raise RuntimeError("Target column not found in CSV. Need Hair_Loss or HairLoss or bald_prob")
...
print("Selected best model:", best, results[best])
